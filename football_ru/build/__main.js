$(document).ready(function() {
    ...
    function createStructure(obj, side) {
        ...
        if (obj.descr) {
            ...
            img.each(function() {
                switch ($(this).attr('alt')) {
                    case 'goal':
                        $(this).attr('src', 'https://js.dooh.xyz/football_ru/server/images/ball.svg');
                        break;
                    case 'redcard':
                        $(this).attr('src', 'https://js.dooh.xyz/football_ru/server/images/redcard.svg');
                        break;
                    case 'yellowcard':
                        $(this).attr('src', 'https://js.dooh.xyz/football_ru/server/images/yellowcard.svg');
                        break;
                }
            })
        } else {
        ...
        switch (obj.status) {
            case 'Не начался':
                elem.find('.js-table').html('<tr><td class="not-start">' + obj.status + '<div> начало в </div><div>'+obj.time+'</div></td></tr>')
                break;
            case 'Техн. победа':
                elem.find('.js-table').html('<tr><td class="custom">' + obj.status + '</td></tr>')
                break;
        }
    ...
    try {
        ...
        $.ajax({
            url: 'https://js.dooh.xyz/football_ru/server/redirect.php',
            beforeSend: function(){
                time = new Date();
